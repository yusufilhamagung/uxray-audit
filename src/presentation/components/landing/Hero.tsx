...
export default function Hero() {
  ...
  const handleAnalyzeClick = async () => {
    ...
    try {
        ...
        if (data.audit_id) {
            router.push(`/audit?id=${data.audit_id}`);
        }
    } catch (e) {
    ...
    <section className="relative overflow-hidden pt-32 pb-16 sm:pb-24 lg:pb-32">
      <div className="section-container flex flex-col items-center justify-center">
        <div className="space-y-8 text-center max-w-4xl mx-auto">
          <div className="inline-flex items-center rounded-full border border-orange-100 bg-orange-50 px-3 py-1 text-sm font-medium text-orange-600">
          ...
          <div className="flex flex-col items-center gap-4 w-full">
             ...
             <div className="bg-slate-100 p-1 rounded-lg inline-flex">
                <button 
                   onClick={() => setMode('url')}
                   className={`px-4 py-1.5 text-sm font-medium rounded-md transition-all ${mode === 'url' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-900'}`}
                >
                  Website URL
                </button>
                <button 
                  onClick={() => setMode('upload')}
                  className={`px-4 py-1.5 text-sm font-medium rounded-md transition-all ${mode === 'upload' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-900'}`}
                >
                  Upload Screenshot
                </button>
             </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 items-center justify-center w-full max-w-lg mx-auto">
                  <div className="relative w-full">
                      ...
                      <input 
                        ...
                        className="w-full rounded-full border border-slate-200 bg-white py-3 pl-10 pr-4 text-sm outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-200 transition-all"
                      />
                  </div>
             ...
             {mode === 'upload' && (
                <div className="flex flex-col sm:flex-row gap-3 items-center justify-center w-full max-w-lg mx-auto">
                  <div className="relative w-full">
                     <label className="flex flex-col items-center justify-center w-full h-[50px] border-2 border-slate-300 border-dashed rounded-full cursor-pointer bg-slate-50 hover:bg-slate-100 transition-colors overflow-hidden relative">
                        {file ? (
                          <div className="flex items-center gap-2 text-sm text-slate-700 px-4">
                             <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-green-500 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>

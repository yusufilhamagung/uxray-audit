...
import type { AuditResult, Category, PageType } from '@/lib/schema';
...
export default function AuditPage() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!file) {
      setError('Mohon unggah screenshot terlebih dahulu.');
      return;
    }
    if (!pageType) {
      ...
      <main className="mx-auto min-h-screen max-w-6xl px-6 pb-20 pt-12">
        <header className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-3xl font-semibold text-slate-900">Audit UX dari Screenshot</h1>
          </div>
        ...
        >
          <div className="card space-y-6 p-6">
            ...
            <div>
              <label className="text-sm font-semibold text-slate-800">URL Website (coming soon)</label>
              <div className="mt-2">
                <input
                  type="url"
                  placeholder="https://contoh.com"
                  className="block w-full rounded-2xl border border-dashed border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-400"
                  disabled
                />
              </div>
            </div>
          ...
          <div className="card flex h-full flex-col items-center justify-center gap-4 p-6">
            ...
            {previewUrl ? (
              <img
                ...
                className="max-h-[320px] w-full rounded-2xl border border-slate-200 object-contain"
              />
            ) : (
